...
import torch
from sam2.build_sam import build_sam2_video_predictor_softmax, build_sam2
...
class MedSAM2Model:
    def __init__(self, model_cfg=None, checkpoint=None, model_type=None, device= torch.device("cuda" if torch.cuda.is_available() else "cpu"), model=None):
        ...
        if self.model_type == 'video':
            print('Model')
            if model:
                self.model = build_sam2_video_predictor_softmax(model=model, device=self.device)
                model.eval()
            else:
                self.model = build_sam2_video_predictor_softmax(model_cfg, checkpoint, device=self.device)
        else:
