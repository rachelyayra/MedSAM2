...
model_cfg = '/scratch_net/ken/radjoe/Projects/Experiments/SAMEXP/configs/sam2.1_hiera_tiny_finetune512.yaml'
checkpoint = '/scratch_net/ken/radjoe/Projects/Experiments/SAMEXP/checkpoints/MedSAM2_latest.pt'

class HQMedSAM2Model:
    def __init__(self, model_cfg, checkpoint, model_type, device= torch.device("cuda" if torch.cuda.is_available() else "cpu")):
        ...
        if self.model_type == 'video':
            self.model = build_sam2_hq_video_predictor(model_cfg, checkpoint, device=self.device)
        else:
