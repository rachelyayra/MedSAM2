...
from models.HQSAM2 import HQSAM2Model
...
def postprocess_save(prediction,label_shape, model_type):
    ...
    if model_type == 'video':
        total_mask = np.zeros(label_shape).squeeze(0).squeeze(0)
...
def predict_and_save(model_cfg, checkpoint, video_path, dataloader,savepath, model_type):
    '''
    Dataloader
    Predict
    Save to directory
    '''
    model = HQSAM2Model(model_cfg, checkpoint, model_type)
    # print(dataloader)
